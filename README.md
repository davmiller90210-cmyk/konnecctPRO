...
That script **`docker cp`**s core Konnecct files (including `hooks.py`, `konnecct_signup.py`, the **entire `crm/api/`** package so scheduler hooks like `crm.api.event` resolve, login/signup templates, the **entire `crm/patches/`** tree, and `crm/patches.txt`) into the `frappe` container, runs **`bench migrate`** (for custom fields such as the first-time password flag), then **`bench clear-cache`** + **`bench restart`**. Copying only `patches.txt` without the matching `.py` files will break **`bench migrate`** with `ModuleNotFoundError` for patch modules. The sync script removes `api/` and `patches/` **as root** (then `chown`s to `frappe`) because **`docker cp`** often leaves **root-owned** files that the `frappe` user cannot delete. Set **`SITE_NAME`** if your site is not `app.konnecct.com` (e.g. `SITE_NAME=app.example.com bash scripts/sync_konnecct_into_docker_frappe.sh`).
