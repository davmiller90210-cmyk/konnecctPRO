		...
		200: function (data) {
			if (data.message == 'Logged In') {
				...
				if (cint(data.message[0]) == 0) {
					...
					login.set_status({{ _("Success") | tojson }}, 'green');
					// Konnecct: server logs the user in; send them to CRM (Frappe default leaves you on #signup).
					var _signup_to = frappe.utils.sanitise_redirect(frappe.utils.get_url_arg("redirect-to")) || "/crm";
					window.location.href = _signup_to;
				}
			}
